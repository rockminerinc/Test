...
static struct opt_table opt_config_table[] = {
	...
	OPT_WITH_ARG("--rmu-freq",
		     set_float_125_to_500, &opt_show_intval, &opt_rock_freq,
		     "Set RockMiner frequency in MHz, range 125-500"),
	OPT_WITH_ARG("--rmu-chip1",
		     set_float_125_to_500, &opt_show_intval, &opt_rock_chip_1,
		     "Set RockMiner frequency in MHz, range 125-500"),
	OPT_WITH_ARG("--rmu-chip2",
		     set_float_125_to_500, &opt_show_intval, &opt_rock_chip_2,
		     "Set RockMiner frequency in MHz, range 125-500"),
		OPT_WITH_ARG("--rmu-chip3",
		     set_float_125_to_500, &opt_show_intval, &opt_rock_chip_3,
		     "Set RockMiner frequency in MHz, range 125-500"),
		OPT_WITH_ARG("--rmu-chip4",
		     set_float_125_to_500, &opt_show_intval, &opt_rock_chip_4,
		     "Set RockMiner frequency in MHz, range 125-500"),
     OPT_WITHOUT_ARG("--rmu-log",
			opt_set_bool, &opt_rock_log,
			"Enable RockMiner display log, default: disabled"),
	OPT_WITHOUT_ARG("--rmu-debug",
			opt_set_bool, &opt_rock_debug,
	...
	OPT_WITH_ARG("--rmu-hard",
		     ...
		     "Set RockMiner diff, range 0-3"),
	OPT_WITH_ARG("--rmu-workmode",
		     set_int_0_to_100, opt_show_intval, &opt_rock_workmode,
		     "Set RockMiner diff, range 0-3"),
#endif
...
static void curses_print_status(void)
{
	...
	if(opt_rock_speed)
		{
		cg_mvwprintw(statuswin, 3, 0, " A:%.0f  R:%.0f  HW:%d  WU:%.1f/m  Freq:%.0fMHz Speed:%dGh/s Dev:%d",
			     total_diff_accepted, total_diff_rejected, hw_errors,
			     total_diff1 / total_secs * 60,opt_rock_freq,opt_rock_speed,most_devices);
		}
	else
		{
	...
	if(opt_rock_workmode == 1)	// balance
		...
		else if(opt_rock_workmode == 2)  // quiet
			{
...
static void curses_print_devstatus(struct cgpu_info *cgpu, int devno, int count)
{
	...
	if (cgpu->status == LIFE_DEAD)
		...
		cg_wprintw(statuswin, "| T:%*.1fC", tpwidth ,cgpu->temp);
			}
		
	if(opt_rock_speed)
		{
		cgpu->speed = (double)((cgpu->total_mhashes / dev_runtime * 1000000ull)/1000000000ull);
		if(cgpu->speed >= opt_rock_speed)	//cgpu->total_mhashes
			{
		cg_wprintw(statuswin, "<OK>"); 
			}
		}
	} else {
		adj_fwidth(cgpu->diff_accepted, &dawidth);
